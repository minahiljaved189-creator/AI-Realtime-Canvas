...
/**
 * "Star" = 1 center node + remaining nodes evenly spaced in a ring around it —
 * matching the assignment's own reference image (center node + 6 surrounding).
 * NOT a 5-pointed star polygon.
 */
function starPositions(count: number, shape: ShapeType): Point[] {
  if (count <= 1) return [{ x: 0, y: 0 }];
  const ringCount = count - 1;
  const outerR = Math.min(maxRadiusFor(shape), 190);
  const points: Point[] = [{ x: 0, y: 0 }];
  for (let i = 0; i < ringCount; i++) {
    const angle = (2 * Math.PI * i) / ringCount - Math.PI / 2;
    points.push({ x: outerR * Math.cos(angle), y: outerR * Math.sin(angle) });
  }
  return points;
}
...
function crossPositions(count: number): Point[] {
  ...
  const perAxis = Math.ceil(count / 2);
  const spacing = arm / Math.max(perAxis, 1);
  const points: Point[] = [];

  // Alternate horizontal/vertical arms outward from center for even, symmetric fill.
  for (let i = 0; i < count; i++) {
    const step = Math.floor(i / 2) + 1;
    const dist = spacing * step;
    if (i % 2 === 0) {
      const sign = Math.floor(i / 4) % 2 === 0 ? 1 : -1;
      points.push({ x: sign * dist, y: 0 });
    } else {
      const sign = Math.floor(i / 4) % 2 === 0 ? 1 : -1;
      points.push({ x: 0, y: sign * dist });
    }
  }
  return points;
}
